...
public class ControlProgramTab extends ControlTab implements Runnable, ActionListener,MouseListener, ItemListener, ChangeListener {
    ...
    private String getPlaybackRequestString(BOPlaybackOption option) {
        ...
        try {
            ...
            SerAlertDialog.alert(
                    ControlMain.getProperty("msg_aPidNotAvailabe")+ 
                    execString.substring(aPidStringIndex, aPidStringIndex + 4),
                    this.getMainView()
            );
            return null;
        }
    ...
    public BOEpg getRunnigEpg(ArrayList epgList) {
        if (epgList != null) {
            ...
            for (int i = 0; i < epgList.size(); i++) {
                ...
                int epgIndex = i - 1;
                if (now.get(GregorianCalendar.DATE) == epgObj.getStartdate()
...
//          Bei IP-Wechsel refreshen, da evtl anderes Box-Image
    ...
    public void reInitBouquetList(JComboBox bouquetsComboBox) {
        if (this.getBouquetList().size() > 0) {
            ...
            this.reInitSender();
            if (!this.firstStart) { this.showFirstSender(); 
//			nur dann anzeigen, wenn bouquets nach dem ersten start gewechselt werden
            }
        }
    }
