			...
			if (tableName == "Epg") {
				...
				if (me.getClickCount()==2) {
					...
					if (ControlMain.getBoxAccess().setTimer("new", timer).equals("ok")) {
						Logger.getLogger("ControlProgramTab").info("Timer �bertragen "+this.getSelectedEpg().getInfo());
					} else {
...
//			Schleife �ber die EPG-Liste um das passende EPG zur epg-event-id zu ermitteln
			for (int i2 = 0; i2<list.size(); i2++) {
				...
				if (epg.getEventId().equals(eventId)) {
					...
					try {
						if (ControlMain.getBoxAccess().setTimer("new", timer).equals("ok")) {
							Logger.getLogger("ControlProgramTab").info("Timer �bertragen "+epg.getInfo());
						} else {
