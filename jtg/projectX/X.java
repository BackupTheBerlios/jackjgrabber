...
static JPanel guiTabProjectX; //DM20032004 081.6 int18 changed
...
static JCheckBox[] cBox = new JCheckBox[55];
...
PATCH vpatch = new PATCH(guiTabProjectX);
		...
		public void mouseClicked(MouseEvent e)
		{
			...
			if (e.getClickCount() > 1)
			{
				if (e.getModifiers() == MouseEvent.BUTTON3_MASK && index > -1) // rename file
				{
					...
					String inputval = JOptionPane.showInputDialog(guiTabProjectX, inchild , "rename " + inparent + inchild, JOptionPane.QUESTION_MESSAGE );

					if (inputval != null && !inputval.equals(""))
					{
						if (new File(inparent + inputval).exists())
						{
							int opt = JOptionPane.showConfirmDialog(guiTabProjectX, "File exists! Overwrite?");
...
protected JPanel buildsplitPanel()
{
	...
	Object[] c18 = { "auto","0xe0","0xe1","0xe2","0xe3","0xe4","0xe5","0xe6","0xe7","0xe8","0xe9","0xea","0xeb","0xec","0xed","0xee","0xef" };
	Object[] c19 = { "auto","0xc0","0xc1","0xc2","0xc3","0xc4","0xc5","0xc6","0xc7","0xc8","0xc9","0xca","0xcb","0xcc","0xcd","0xce","0xcf" };
	Object[] c22 = { "auto","0x80","0x81","0x82","0x83","0x84","0x85","0x86","0x87","0x88","0x89","0x8a","0x8b","0x8c","0x8d","0x8e","0x8f" };
	String[] tx = { " MPEG Video"," MPEG Audio A"," MPEG Audio B"," MPEG Audio C"," AC-3 Audio (sub ID, if available)" };

	JPanel idPanel = new JPanel();
	idPanel.setBorder(BorderFactory.createTitledBorder("settings for main IDs"));
	idPanel.setLayout ( new ColumnLayout() );

	comBox[18] = new JComboBox(c18);
	comBox[18].setMaximumRowCount(6);
	comBox[18].setPreferredSize(new Dimension(70,25));
	comBox[18].setSelectedItem("auto");

	JPanel[] id1 = new JPanel[5];
	id1[0] = new JPanel();
	id1[0].setLayout(new BoxLayout(id1[0], BoxLayout.X_AXIS));
	id1[0].add(comBox[18]);
	id1[0].add(new JLabel(tx[0]));
	idPanel.add(id1[0]);
	idPanel.setToolTipText("if more than one stream in source, specify the main ID (auto means: take what comes at first)");

	comBox[20] = new JComboBox(c22);
	comBox[20].setMaximumRowCount(6);
	comBox[20].setPreferredSize(new Dimension(70,25));
	comBox[20].setSelectedItem("auto");

	id1[1] = new JPanel();
	id1[1].setLayout(new BoxLayout(id1[1], BoxLayout.X_AXIS));
	id1[1].add(comBox[20]);
	id1[1].add(new JLabel(tx[4]));
	idPanel.add(id1[1]);
	...
	cBox[7] = new JCheckBox("write all audio data");
...
protected JPanel buildaudioPanel() {
	...
	//RButton[2] = new JRadioButton("normalize");
	RButton[2] = new JRadioButton("amplify by factor: ");
	RButton[2].setPreferredSize(new Dimension(150,20));
	RButton[2].setMaximumSize(new Dimension(150,20));
	RButton[2].setToolTipText("factor 0 & 100 does nothing, else:  down - 100 - up, std=150");

	exefield[8] = new JTextField("150");
...
protected JPanel buildoptionPanel() {
	...
	Object[] scan = { "1024000","1536000","2048000","2560000","3072000" };
	comBox[37]=new JComboBox(scan);
	comBox[37].setSelectedIndex(0);
...
class MenuListener implements ActionListener
{
	public void actionPerformed(ActionEvent e)
	{
		...
		else if (actName.equals("add"))
		{
			...
			int retval = chooser.showDialog(guiTabProjectX, null);
...
{
	guiTabProjectX.setVisible(visible);
}
...
public void run() {
	...
	if (options[33]==-1) {
		...
		if (cBox[8].isSelected()) {
			Msg("-> add time offset to audio/ttx :");
			Msg("\t"+(options[28]/90)+" ms (if it follows on a video)");
		}
...
public void pesparse(String file, String vptslog, int ismpg) {
	...
	for (int a=0;a<PESdemuxlist.size();a++) {
		...
		case 4: //DM23022004 081.6 int18 add
			...
			processLPCM(values);
			break;
		case 5: //DM23022004 081.6 int18 add
			Msg("");
			Msg("--> RLE Subpicture (SubID 0x"+Integer.toHexString(demux.subID()).toUpperCase()+")");
	...
	while (true) {
		...
		while ( count < size )  {     // till fileend (64bit size)
			...
			if (!dontread || !cBox[53].isSelected())
			{
 			...
 			/*** check 0x47 sync  ***/
 			if ( push189[0]!=0x47 || push189[188]!=0x47 )
			{
...
public void mpt(String[] args) {
	...
	while ( processAudio(args) ) {
		options[17] &= ~0xCL;
		Msg("!> restart audio processing, forced by changes @ output frame "+(options[17]>>>18));
		yield();
...
public boolean processAudio(String[] args) {
	...
	while (true) {
		...
		******************************/
		...
		if (args[2].equals("mp") && cBox[50].isSelected())
		{
			...
			MPADecoder.MULTIPLY = (RButton[2].isSelected()) ? (Double.parseDouble(exefield[8].getText().toString())/100.0) : 0;
	...
	String comparedata = "Audio "+(NoOfAudio++)+":\t"+frame_counter+" Frames\t"+tc+"\t"+cb+"/"+ce+"/"+cc+"/"+cd;
	...
	case 3: { 
		...
		if (audioout1.length()<100) 
			...
			InfoAtEnd.add("MP1-"+comparedata+"\t "+mp1name); 
		}
		if (audioout2.length()<100) 
			audioout2.delete();
...
***/
	...
	{ 
		...
		InfoAtEnd.add("LPCM " + (NoOfPictures++) + ":\t" + samples + " packs\t\t " + pcmfile1);
	}
...
class PIDdemux {
	...
	public void write(byte[] data, boolean header) {    // data = 1 pespacket from demux
		...
		if ( ptslength>0 && data.length>=ptslength) {    // read pts, if available
			...
			if (lastPTS != pts){ //DM22122003 081.6 int09 change
...
class makeVDR {
	...
	public long[] writeMPG(byte[] data, long[] options, PIDdemux demux, int overhead) {    // data = 1 pespacket from demux
		...
		if (data.length>overhead) { 
			...
			if (first) {
				...
				//options[39]+=120; 
				//options[41]+=120;  // placeholder for systemheader
				first=false;
			}
	...
	/******* write VDR ******/
	public long[] writeVDR(byte[] data, long[] options, PIDdemux demux, int overhead) {    // data = 1 pespacket from demux
		try 
		{
		int type = demux.getType(), stream = demux.getStreamType();
		int newID = demux.getnewID();       // read new mapped ID
		//   if (type==0 && newID>0x80) return options;   // return, nor more subIDs accept
		if (type==0 && newID!=ac3ID) 
			return options;   // return, wrong ac3 subIDs not accept
		...
		if (data.length>overhead) { 
			...
			if (type==0 && stream==2) {  // skip Ac-3 sub-id and only write one stream
				len-=4;
				int size = len-6;
				int head = 9+(0xFF&data[8]);
				data[4] = (byte)(size>>>8);    // correct length info
	...
	public void close() { 
		...
		if (makempg) { 
			...
			/**** add stuff ***/
			mpg.writeInt(0x1be);
			//mpg.writeShort((short)(0x7FA-insert));
			//mpg.write(fills,0,(0x7FA-insert));
			//mpg.writeShort((short)(128-insert));
			//mpg.write(fills,0,(128-insert));
		...
		if (toTS && cBox[35].isSelected()) {
			...
			new File(name).renameTo(new File(name1));
			name = name1;
	...
	if (error) {
		...
		/*** H Resolution request for auto cut  ***/
			...
			**/
			...
			brm.surf.update(vbr,frametypebuffer.toByteArray(),sms.format(new java.util.Date((options[48]+options[42])/90)).substring(0,8));
...
public static void setvideoheader(String videofile, String logfile) {
	...
	if (new File(logfile).exists()) {
		...
		Msg("");
		Msg("video length: "+options[7]+"f @ "+vt);
		//Msg("-> GOP structure summary: min. "+clv[8]+", max. "+clv[9]+" fields");
		InfoAtEnd.add(".Video:\t"+options[7]+" Frames\t"+vt+"\t\t "+new File(videofile));
	}
